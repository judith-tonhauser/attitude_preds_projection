...
# plot at-issueness by prior (4-way distinction) ----
...
# plot at-issueness by prior (3-way distinction) ----
...
ggplot(ai.means, aes(x=verb, y=Mean, fill=prior)) + 
  ...
  xlab("Predicate") +
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1))
ggsave("../graphs/3way-means-nai-by-predicate-and-prior.pdf",height=4,width=7)

## plot projection by prior (4-way predicate distinction) ----
...
## plot projection by prior (3-way predicate distinction) ----
...
ggplot(proj.means, aes(x=verb, y=Mean, fill=prior)) + 
  ...
  xlab("Predicate") +
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1))
ggsave("../graphs/3way-means-projectivity-by-predicate-and-prior.pdf",height=4,width=7)
...
toplot = tmp.ai %>%
  left_join(tmp.proj, by=c("short_trigger","prior")) %>%
...
ggplot(toplot[toplot$short_trigger != "MC",], aes(x=AIMean,y=ProjMean,fill=VeridicalityGroup)) +
  ...
  scale_fill_manual(values=fill_cols) +
  geom_abline(intercept=0,slope=1,linetype="dashed",color="gray60") +
  geom_errorbar(aes(ymin=ProjYMin,ymax=ProjYMax,alpha = prior == "H")) +
  geom_errorbarh(aes(xmin=AIYMin,xmax=AIYMax,alpha = prior == "H")) +
  guides(fill=FALSE) +
  theme(legend.position = "none") +
  geom_text_repel(aes(label=short_trigger),color=rep(cols2$Colors),alpha=1,size=4,
                  data = toplot[toplot$prior == "L",]) +
  #geom_text_repel(aes(label=short_trigger),color=rep(cols2$Colors,each=2),alpha=1,size=4) +
  ylab("Mean projectivity rating") +
  xlab("Mean not-at-issueness rating") 
ggsave("../graphs/mean-projectivity-by-at-issueness-and-prior2.pdf",height=5,width=5)

### plot mean at-issueness ratings against mean projectivity ratings by prior (3-way distinction) ----
...
#fill_cols = c("darkorchid","black","gray60","tomato1","dodgerblue","black")
fill_cols = c("darkorchid","black","tomato1","black")
...
ggplot(toplot[toplot$short_trigger != "MC",], aes(x=AIMean,y=ProjMean,fill=VeridicalityGroup)) +
  ...
  ylab("Mean projectivity rating") +
  xlab("Mean not-at-issueness rating") 
ggsave("../graphs/3way-mean-projectivity-by-at-issueness-and-prior2.pdf",height=6,width=6)

### plot mean at-issueness ratings against mean projectivity ratings NO PRIOR (3-way distinction) ----
proj.means = cd %>%
